#!/usr/bin/env python3
"""bench_flux.py - Run on: client machine with GPU

Benchmark script for FLUX.2-klein-4B image generation using Hugging Face Diffusers.
Measures image generation throughput (images/second) on target hardware.

Requirements:
    pip install torch diffusers transformers accelerate tqdm --break-system-packages

Usage:
    python bench_flux.py --num-images 50 --warmups 5 --steps 4
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark FLUX.2-klein-4B image generation performance",
...
def save_csv(args, results: list, gpu_info: dict, timestamp: str) -> str:
    ...
    csv_file = f"{output_dir}/flux_bench_{gpu_info['name_sanitized']}_{model_sanitized}_n{args.num_images}_{timestamp}.csv"
...
def main():
    ...
    print("=" * 60)
    print("FLUX.2-klein-4B IMAGE GENERATION BENCHMARK")
    print("=" * 60)
