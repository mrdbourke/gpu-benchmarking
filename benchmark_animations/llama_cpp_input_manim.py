"""
llama.cpp Prompt Throughput Animated Comparison

This Manim script generates an animated bar chart comparing prompt token throughput
(true_prompt_throughput) across models for NVIDIA GB10 and RTX 4090 GPUs.

Command: manim -pql llama_cpp_input_manim.py LlamaCppInputThroughput
Custom flags:
...
class LlamaCppInputThroughput(Scene):
    def construct(self):
        models = [
            "gpt-oss-20b (text)",
        ...
        gb10_values = [2703.36, 1037.78, 799.44, 2478.36]
        rtx_values = [6035.78, None, 2459.23, 8414.23]
        ...
        title = Text("llama.cpp Inference: Prompt Throughput", font_size=42, weight=BOLD)
        ...
        subtitle = Text("Prompt Tokens per Second (tok/s) · ↑ Higher is better", font_size=24, color=GRAY)
        if subtitle.width > canvas.safe_width:
            subtitle.scale(canvas.safe_width / subtitle.width * 0.95)
        subtitle.next_to(title, DOWN, buff=0.2)

        metadata = VGroup(
            Text("Metric: true_prompt_throughput (aggregate prompt tokens / total prompt time)", font_size=18, color=GRAY),
            Text("100 prompts · input range 50-1000 tokens · output range 50-1000 tokens", font_size=18, color=GRAY),
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="llama.cpp Prompt Throughput Animation")
    ...
    if args.preview:
        cmd.append("-p")
    cmd.extend([__file__, "LlamaCppInputThroughput"])
