"""
Image Generation Throughput Comparison

Compares images-per-second across GPUs for Flux.2-klein-4B and Z-Image-Turbo.

Command: manim -pql image_generation_manim.py ImageGenerationThroughput
Custom flags:
...
class ImageGenerationThroughput(Scene):
    def construct(self):
        models = [
            "Flux.2-klein-4B",
            "Z-Image-Turbo",
        ...
        gb10_values = [0.2380, 0.0598]
        rtx_values = [0.7764, 0.2053]
        faster_ratios = [round(rtx_values[0] / gb10_values[0], 1), round(rtx_values[1] / gb10_values[1], 1)]
        ...
        title = Text("Image Generation: Throughput", font_size=42, weight=BOLD)
        ...
        subtitle = Text("Images per Second (img/s) · ↑ Higher is better", font_size=24, color=GRAY)
        if subtitle.width > canvas.safe_width:
            subtitle.scale(canvas.safe_width / subtitle.width * 0.95)
        subtitle.next_to(title, DOWN, buff=0.2)

        metadata = VGroup(
            Text("Resolution 1024x1024 · 100 images per run", font_size=18, color=GRAY),
            Text("Flux: 4 steps · Z-Image-Turbo: 9 steps", font_size=18, color=GRAY),
        ...
        footer = Text(f"RTX 4090 is faster at generating images with Flux.2-klein-4B by {faster_ratios[0]}x and with Z-Image-Turbo by {faster_ratios[1]}x.", font_size=14, color=GRAY)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Image Generation Throughput Animation")
    ...
    if args.preview:
        cmd.append("-p")
    cmd.extend([__file__, "ImageGenerationThroughput"])
