"""
llama.cpp vs vLLM Generation Throughput Comparison

Compares generation token throughput between llama.cpp and vLLM
for the same models and GPUs.

Command: manim -pql llama_cpp_vs_vllm_output_manim.py LlamaCppVsVLLMOutputThroughput
Custom flags:
...
class LlamaCppVsVLLMOutputThroughput(Scene):
    def construct(self):
        models = [
            "gpt-oss-20b (text)",
        ...
        series = [
            {
                ...
                "color": "#1f77b4",
                "values": [88.04, 35.48, 33.07],
            },
            {
                "label": "GB10 · vLLM",
                "color": "#6baed6",
                "values": [230.00, 220.00, 191.00],
            },
            {
                "label": "RTX 4090 · llama.cpp",
                "color": "#ff7f0e",
                "values": [242.54, 132.22, 122.83],
            },
            {
                "label": "RTX 4090 · vLLM",
                "color": "#ffb570",
                "values": [584.00, 580.00, 392.00],
            },
        ]

        canvas = CanvasLayout(config, margin=0.5)

        title = Text("llama.cpp vs vLLM: Generation Throughput", font_size=40, weight=BOLD)
        ...
        metadata = VGroup(
            Text("llama.cpp: true_gen_throughput · vLLM: output_token_throughput", font_size=18, color=GRAY),
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="llama.cpp vs vLLM Output Throughput Animation")
    ...
    if args.preview:
        cmd.append("-p")
    cmd.extend([__file__, "LlamaCppVsVLLMOutputThroughput"])
