"""
LLM Fine-Tuning Training Time Comparison

Compares total training time across GPUs for Gemma-3-270M fine-tuning.

Command: manim -pql llm_finetune_time_manim.py LLMFineTuneTrainingTime
Custom flags:
...
class LLMFineTuneTrainingTime(Scene):
    def construct(self):
        ...
        title = Text("LLM Fine-Tuning: Training Time", font_size=42, weight=BOLD)
        ...
        metadata = VGroup(
            Text("Dataset: FoodExtract-1k · 3 epochs · batch size 8", font_size=18, color=GRAY),
            Text("Sequence length 512 · full fine-tune", font_size=18, color=GRAY),
        ...
        label_right_x = canvas.safe_left + 3.0
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="LLM Fine-Tuning Training Time Animation")
    ...
    if args.preview:
        cmd.append("-p")
    cmd.extend([__file__, "LLMFineTuneTrainingTime"])
