"""
vLLM Token Generation Throughput Animated Comparison

This Manim script generates an animated bar chart comparing token generation
(output) per second across different models for NVIDIA GB10 and RTX 4090 GPUs.

Run from: /home/claude/ (or wherever this file is saved)
Command: manim -pql vllm_output_manim.py VLLMOutputThroughput
...
Examples:
    ...
    manim -pql vllm_output_manim.py VLLMOutputThroughput
    
    # No fadeout (default) - via python wrapper
    python vllm_output_manim.py -p -ql
    
    # With fadeout - via python wrapper
    python vllm_output_manim.py -p -ql --fadeout
    
Output: media/videos/vllm_output_manim/480p15/VLLMOutputThroughput.mp4
"""
...
class VLLMOutputThroughput(Scene):
    def construct(self):
        # Data
        models = [
            "gpt-oss-20b",
        ...
        # GB10 output throughput (tok/s)
        gb10_output = [230, 116, 230, 220, 191, 150, 130, 65, 62]
        
        # RTX 4090 output throughput (tok/s), 0 for NA values
        rtx4090_output = [584, 0, 0, 580, 392, 0, 0, 0, 0]
        ...
        # === Title Section ===
        title = Text("vLLM Inference: Token Generation Throughput", font_size=42, weight=BOLD)
        ...
        subtitle = Text("Output Tokens per Second (tok/s) · ↑ Higher is better", font_size=24, color=GRAY)
        ...
        max_value = max(max(gb10_output), max(rtx4090_output))
        ...
        max_bar_width = canvas.safe_right - bar_start_x - 1.0  # Leave room for values
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="vLLM Output Throughput Animation")
    ...
    if args.preview:
        cmd.append("-p")
    cmd.extend([__file__, "VLLMOutputThroughput"])
